...
namespace Server.Items
{
    ...
    public abstract class BaseAxe : BaseMeleeWeapon, IHarvestTool
    {
        ...
        public override void OnHit(Mobile attacker, IDamageable defender, double damageBonus)
        {
            ...
            if (!Core.AOS && defender is Mobile mobileDefender && (attacker.Player || attacker.Body.IsHuman) && this.Layer == Layer.TwoHanded && (attacker.Skills[SkillName.Anatomy].Value / 400.0) >= Utility.RandomDouble())
            {
                // Check if the defender is not a player (for PvM only)
                if (!(mobileDefender is PlayerMobile))
                {
                    StatMod mod = mobileDefender.GetStatMod("Concussion");

                    if (mod == null)
                    {
                        mobileDefender.SendMessage("You receive a concussion blow!");
                        mobileDefender.AddStatMod(new StatMod(StatType.Int, "Concussion", -(((Mobile)mobileDefender).RawInt / 2), TimeSpan.FromSeconds(30.0)));
