...
namespace Server.Items
{
	public abstract class BaseRanged : BaseMeleeWeapon
	{
		...
		public override TimeSpan OnSwing(Mobile attacker, IDamageable damageable)
		{
			...
			{
				int dexPenalty = (int)(100 - ((PlayerMobile)attacker).Dex * 12);
                nextShoot = ((PlayerMobile)attacker).LastMoveTime + dexPenalty + 1700;
				// attacker.SendMessage("Ready to shoot; stop moving! {0}", Core.TickCount - nextShoot);
			}
            else
