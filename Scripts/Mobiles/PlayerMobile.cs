...
private void DismountIfMounted(Mobile mobile)
{
    if (mobile.Mounted)
    {
        mobile.Mount.Rider = null;
        // Console.WriteLine($"Dismounted: {mobile.Name}");
    }
}

public override void DoHarmful(IDamageable damageable, bool indirect)
{
    Console.WriteLine($"DoHarmful called with indirect: {indirect}, Spell: {(damageable as Mobile)?.Spell?.GetType().Name}, Damageable: {damageable?.GetType().Name}");
    base.DoHarmful(damageable, indirect);

    if (damageable is PlayerMobile defender)
    {
        Console.WriteLine($"Spell effect applied on defender: {defender.Spell?.GetType().Name ?? "None"}");

        if (this is Mobile attacker)
        {
            Console.WriteLine($"Defender's current hits before potential damage: {defender.Hits}, Max hits: {defender.HitsMax}");

            bool shouldDismountForSpell = false;
            bool shouldDismountForDamage = false;

            // Check for harmful spell
            if (attacker.Spell != null && IsHarmfulSpell(attacker.Spell))
            {
                shouldDismountForSpell = true;
                Console.WriteLine("Dismounting due to harmful spell effect.");
            }

            // Check for damage dealt immediately
            var damageEntry = defender.DamageEntries.FirstOrDefault(de => de.Damager == this);
            if (damageEntry != null)
            {
                shouldDismountForDamage = true;
                Console.WriteLine("Dismounting due to direct harm detected immediately.");
            }
            else
            {
                // If no immediate damage, check again after a brief delay
                Console.WriteLine("No immediate damage detected, checking again after delay.");
                Timer.DelayCall(TimeSpan.FromMilliseconds(1000), () => 
                {
                    var delayedDamageEntry = defender.DamageEntries.FirstOrDefault(de => de.Damager == this);
                    if (delayedDamageEntry != null && !m_HasDismountedThisAction)
                    {
                        m_HasDismountedThisAction = true;
                        // Console.WriteLine("Dismounting due to delayed damage check.");
                        DismountIfMounted(attacker);
                        DismountIfMounted(defender);
                    }
                    else
                    {
                        Console.WriteLine("No damage detected even after delay.");
                    }
                });
            }

            // Dismount for spell regardless of previous dismount
            if (shouldDismountForSpell)
            {
                DismountIfMounted(attacker);
                DismountIfMounted(defender);
                // Reset flag since a new spell action is starting
                m_HasDismountedThisAction = false;
            }
            // Dismount for damage only if not already dismounted for damage this action
            else if (shouldDismountForDamage && !m_HasDismountedThisAction)
            {
                m_HasDismountedThisAction = true;
                DismountIfMounted(attacker);
                DismountIfMounted(defender);
            }
            else if (!shouldDismountForSpell && !shouldDismountForDamage)
            {
                Console.WriteLine("No dismount; no immediate effect or damage detected.");
            }

            Console.WriteLine($"Defender's current hits after potential damage: {defender.Hits}");

            // Reset flag for damage dismounting, but not for spells
            if (!shouldDismountForSpell)
            {
                m_HasDismountedThisAction = false;
            }
    ...
    {
        Console.WriteLine("Spell effect applied: None");
    }

    if (ViceVsVirtueSystem.Enabled && (ViceVsVirtueSystem.EnhancedRules || Map == Faction.Facet) && damageable is Mobile mobile)
    {
...
// Existing field
private bool m_HasDismountedThisAction = false;

private bool IsHarmfulSpell(ISpell spell)
{
    // List of spells known to be harmful but not causing immediate damage
    Type[] harmfulSpells = new Type[]
    {
        typeof(ClumsySpell),
        typeof(WeakenSpell),
        typeof(FeeblemindSpell),
        typeof(CurseSpell),
        typeof(Server.Spells.Third.PoisonSpell),
        typeof(Server.Spells.Fifth.PoisonFieldSpell),
        // Add any other spells here as needed
    };

    return harmfulSpells.Contains(spell.GetType());
}
